...
public class HourlyForecastView extends View implements ScrollWatcher {
    ...
    public HourlyForecastView(final Context context, @Nullable AttributeSet attrs,
                              int defStyleAttr) {
    ...
    private void initDefValue() {
        ...
        bitmapHeight = 1 / 2f * (2 * defHeightPixel - lowestTempHeight)
                + DisplayUtil.dp2px(mContext, 2);//- 给文字留地方
        bitmapXY = 18;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //获取测量模式
        //注意 HorizontalScrollView的子View 在没有明确指定dp值的情况下widthMode总是MeasureSpec.UNSPECIFIED
...
//                int Y = getTempBarY(i);
    ...
    private void drawBitmaps(Canvas canvas) {
        ...
        for (int i = 0; i < dashLineList.size() - 1; i++) {
            ...
            if (code.contains("d")) {
                bd = (BitmapDrawable) mContext.getResources().getDrawable(IconUtils
                        .getDayIconDark(code.replace("d", "")));
            } else {
                bd = (BitmapDrawable) mContext.getResources().getDrawable(IconUtils
                        .getNightIconDark(code.replace("n", "")));
            }
...
//            canvas.drawText(text, drawPoint, bitmapHeight + bitmap.getHeight()
//            + 100 / 3f, textPaint);
    ...
    private void drawLines(Canvas canvas) {
        ...
        Shader mShader = new LinearGradient(0, 0, 0, getHeight(), shadeColors,
    ...
    private void drawDashLine(List<Float> dashWidth, List<Float> dashHeight, Canvas canvas) {
        if (dashHeight != null && dashHeight.size() > 1) {
            for (int i = 1; i < dashHeight.size() - 1; i++) {
                canvas.drawLine(dashWidth.get(i), dashHeight.get(i) + 3, dashWidth.get(i),
    ...
    public float tempHeightPixel(float tmp) {
        float res = ((tmp - lowestTemp) / (highestTemp - lowestTemp)) *
                (highestTempHeight - lowestTempHeight) + lowestTempHeight;
